#!/usr/bin/env python3
"""Fetch more specific dinosaur images from Wikimedia Commons."""

import json
import os
import re
import time
import urllib.parse
import urllib.request
from pathlib import Path

WIKI_API = "https://commons.wikimedia.org/w/api.php"
MASTER_DIR = Path("assets/master")
WEB_DIR = Path("assets/web")
MANIFEST_PATH = Path("data/assets-manifest.json")

# Very specific dinosaur search terms - museum exhibits and famous specimens
SEARCH_TERMS = [
    "Tyrannosaurus skeleton museum",
    "Triceratops museum exhibit",
    "Stegosaurus stenops skeleton",
    "Brachiosaurus altithorax",
    "Allosaurus fragilis skull",
    "Dracorex hogwartsia",
    "Edmontosaurus skeleton",
    "Giganotosaurus carolinii",
    "Hadrosaurus skeleton",
    "Kentrosaurus museum",
    "Lambeosaurus skull",
    "Maiasaura skeleton",
    "Nigersaurus skeleton",
    "Oviraptor nest eggs",
    "Quetzalcoatlus reconstruction",
    "Dilophosaurus fossil",
    "Utahraptor fossil",
    "Zhuchengtyrannus magnus",
    "Torosaurus skull",
    "Styracosaurus museum",
    "Corythosaurus skull",
    "Euoplocephalus skeleton",
    "Gastonia skeleton",
    "Herrerasaurus ischigualastensis",
    "Massospondylus fossil",
    "Plateosaurus skeleton museum",
    "Camarasaurus skeleton",
    "Coelophysis ghost ranch",
    "Deinonychus antirrhopus",
    "Gorgosaurus skeleton",
]

FREE_LICENSES = [
    "pd", "public domain", "cc0", "cc-zero",
    "cc-by-sa", "cc-by", "gfdl"
]

def load_existing_manifest():
    if MANIFEST_PATH.exists():
        with open(MANIFEST_PATH) as f:
            return json.load(f)
    return {"version": 2, "updatedAt": "", "policy": {}, "assets": []}

def get_existing_filenames():
    return {f.name for f in MASTER_DIR.glob("*")}

def search_images(term, limit=10):
    params = {
        "action": "query",
        "format": "json",
        "generator": "search",
        "gsrnamespace": "6",
        "gsrsearch": f"filetype:bitmap {term}",
        "gsrlimit": str(limit),
        "prop": "imageinfo",
        "iiprop": "url|extmetadata|size",
        "iiurlwidth": "800"
    }
    url = f"{WIKI_API}?{urllib.parse.urlencode(params)}"
    
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "DinoLabMashup/1.0"})
        with urllib.request.urlopen(req, timeout=30) as resp:
            data = json.loads(resp.read().decode())
            pages = data.get("query", {}).get("pages", {})
            return list(pages.values())
    except Exception as e:
        print(f"  Search error: {e}")
        return []

def is_free_license(license_str):
    if not license_str:
        return False
    lower = license_str.lower()
    return any(lic in lower for lic in FREE_LICENSES)

def sanitize_filename(title):
    name = re.sub(r"^File:", "", title, flags=re.IGNORECASE)
    name = re.sub(r"[^a-zA-Z0-9._-]", "-", name).lower()
    name = re.sub(r"-+", "-", name).strip("-")
    return name[:80] + ".jpg"

def download_image(url, dest):
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "DinoLabMashup/1.0"})
        with urllib.request.urlopen(req, timeout=60) as resp:
            with open(dest, "wb") as f:
                f.write(resp.read())
        return True
    except Exception as e:
        print(f"  Download failed: {e}")
        return False

def main():
    import shutil
    MASTER_DIR.mkdir(parents=True, exist_ok=True)
    WEB_DIR.mkdir(parents=True, exist_ok=True)
    
    existing = get_existing_filenames()
    new_assets = []
    
    print(f"Starting with {len(existing)} existing images")
    print(f"Searching {len(SEARCH_TERMS)} specific dino terms...\n")
    
    for i, term in enumerate(SEARCH_TERMS):
        print(f"[{i+1}/{len(SEARCH_TERMS)}] {term}")
        
        results = search_images(term, limit=6)
        time.sleep(1.2)
        
        for page in results:
            title = page.get("title", "")
            info = page.get("imageinfo", [{}])[0]
            meta = info.get("extmetadata", {})
            
            license_str = meta.get("LicenseShortName", {}).get("value", "")
            if not is_free_license(license_str):
                continue
            
            width = info.get("width", 0)
            height = info.get("height", 0)
            if width < 400 or height < 300:
                continue
            
            thumb_url = info.get("thumburl", info.get("url", ""))
            source_url = info.get("descriptionurl", "")
            if not thumb_url:
                continue
            
            filename = sanitize_filename(title)
            if filename in existing:
                continue
            
            master_path = MASTER_DIR / filename
            if master_path.exists():
                continue
            
            print(f"  ✓ {filename[:50]}...")
            if download_image(thumb_url, master_path):
                shutil.copy(master_path, WEB_DIR / filename)
                
                creator = meta.get("Artist", {}).get("value", "Unknown")
                creator = re.sub(r"<[^>]+>", "", creator)[:80]
                
                desc = meta.get("ImageDescription", {}).get("value", "")
                desc = re.sub(r"<[^>]+>", "", desc)[:150] or f"Image of {term}"
                
                asset = {
                    "id": f"wmc-{filename.replace('.jpg', '')}",
                    "category": "dinosaur-reconstruction",
                    "title": title.replace("File:", ""),
                    "subject": term.split()[0],
                    "creator": creator,
                    "sourceUrl": source_url,
                    "license": license_str,
                    "status": "ready",
                    "formats": {
                        "master": f"assets/master/{filename}",
                        "web": f"assets/web/{filename}"
                    },
                    "altText": desc
                }
                new_assets.append(asset)
                existing.add(filename)
                
                time.sleep(0.4)
    
    print(f"\n✅ Downloaded {len(new_assets)} new images!")
    print(f"Total in folder: {len(list(MASTER_DIR.glob('*')))}")
    
    # Output new assets as JSON for easy append
    if new_assets:
        print("\nNew assets JSON:")
        for asset in new_assets[:5]:
            print(f"  - {asset['subject']}: {asset['title'][:40]}...")

if __name__ == "__main__":
    main()
